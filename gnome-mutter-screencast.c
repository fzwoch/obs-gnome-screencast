...
static void start(data_t* data)
{
	...
	GVariant* stream_res = g_dbus_connection_call_sync(dbus,
		...
		"RecordMonitor",
		g_variant_new_parsed("('', {'cursor-mode' : <%u>})", obs_data_get_bool(data->settings, "cursor") ? 1 : 0),
		NULL,
		G_DBUS_CALL_FLAGS_NONE,
		-1,
		NULL,
	...
	GVariant* stream_res = g_dbus_connection_call_sync(dbus,
		...
		"org.gnome.Mutter.ScreenCast.Session",
		"RecordWindow",
		g_variant_new_parsed("({'window-id' : <%t>, 'cursor-mode' : <%u>},)", 0, obs_data_get_bool(data->settings, "cursor") ? 1 : 0),
		NULL,
		G_DBUS_CALL_FLAGS_NONE,
		-1,
		NULL,
